...
open class UICollectionGridViewLayout: UICollectionViewLayout {
    ...
    open weak var delegate: UICollectionGridViewLayoutDelegate? {
        return self.collectionView?.dataSource as? UICollectionGridViewLayoutDelegate
    }
    ...
    override open func prepare() {
        ...
        if itemsSize.count != delegate?.titlesCols().count {
            calculateItemsSize()
        }
        
        for section in 0 ..< (collectionView?.numberOfSections)! {
            var sectionAttributes: [UICollectionViewLayoutAttributes] = []
            for index in 0 ..< (delegate?.titlesCols().count ?? 0) {
                ...
                if column == delegate?.titlesCols().count {
    ...
    func calculateItemsSize() {
        ...
        var index = (delegate?.titlesCols().count ?? 0) - 1
        while index >= 0 {
    ...
    func sizeForItemWithColumnIndex(columnIndex: Int, remainingWidth: CGFloat) -> CGSize {
        let columnString = delegate?.titlesCols()[columnIndex]
        //根据列头标题文件，估算各列的宽度
